...
with st.sidebar:
    ...
    gemini_api_key = st.text_input(
        ...
        value=default_key,
        type="password",
        help="Set the GEMINI_API_KEY environment variable to avoid pasting it here each time."
    )
    st.caption(f"Primary model: `{GEMINI_MODEL_PRIMARY}`  \nFallback model: `{GEMINI_MODEL_FALLBACK}`")
    st.caption(
        "Never commit real keys into code - use an environment variable."
    )
...
data = pd.read_csv("diabetes_multiclass.csv")
